...
/*   Updated: 2019/08/27 16:25:25 by qmebble          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		push_names_ro_render(t_sum s)
{
	...
	Courier = TTF_OpenFont("/Library/Fonts/Courier New.ttf", 35);
	if (Courier == NULL)
	{
	...
	while (current_vis_room && current_usual_room)
	{
		if (current_vis_room->num == current_usual_room->n)
		{
			name_rect.h = 50;
			name_rect.w = 50;
			name_rect.x = current_vis_room->room.x;
			name_rect.y = current_vis_room->room.y;
			SDL_RenderCopyF(g_main_render, SDL_CreateTextureFromSurface(g_main_render, TTF_RenderText_Solid(Courier, current_usual_room->name, Black)), NULL, &name_rect);
			current_vis_room = current_vis_room->next;
		}
		current_usual_room = current_usual_room->next;
	}
...
void		push_ant_texture_to_render(SDL_FRect pos)
{
	...
	ant_surface = IMG_Load("Visualise/IMG/ant.png");
	if (ant_surface == NULL)
	{
